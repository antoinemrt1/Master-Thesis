...
# /!\ Ce YAML DOIT pointer vers Train_Init (les 20% du dataset)
DATA_INIT_YAML = Path("/linux/antoimartin/v2/data_init.yaml").resolve()
...
def train_initial_model(model_type, imgsz, batch, device):
    ...
    try:
        results = model.train(
            ...
            name=f"baseline_init_20pct_{Path(model_type).stem}_{imgsz}", 
            exist_ok=True,
        ...
        best_pt = MODELS_DIR / f"baseline_init_20pct_{Path(model_type).stem}_{imgsz}" / "weights" / "best.pt"
