...
# Le data.yaml standard pour evaluer sur le Test set global
DATA_YAML = BASE_DIR / "data.yaml"

# MODIFICATION : On utilise le nouveau modele entraine sur 20%
BASE_MODEL_PATH = BASE_DIR / "code/trained_models/baseline_init_20pct_yolov8l_1024/weights/best.pt"

# MODIFICATION : Le pool d'images non etiquetees est maintenant dans Unlabeled_Pool
MISSION_POOL_DIR = BASE_DIR / "dataset/images/Unlabeled_Pool"
VAL_DIR = BASE_DIR / "dataset/images/val"
TEST_DIR = BASE_DIR / "dataset/images/test"

# MODIFICATION : On change V4 en V5 pour isoler les nouvelles experiences du nouveau split
RESULTS_DIR = BASE_DIR / "code/AL_Experiments_V5"
...
def execute_single_run(strategy, percent, missions, imgsz, device, run_id, all_images):
    ...
    for m in range(missions):
        ...
        for img, _ in selected:
            ...
            # MODIFICATION : On va chercher les labels dans labels/Unlabeled_Pool
            lbl = BASE_DIR / "dataset/labels/Unlabeled_Pool" / (img.stem + ".txt")
        ...
        train_model.train(
            data=str(al_yaml),
            epochs=10, # Moins depochs pour eviter le sur-apprentissage
            patience=0,
            freeze=10, # Gele le Backbone
            lr0=0.001,
            imgsz=imgsz,
            batch=4,
            device=device,
...
# ==============================
# GESTIONNAIRE DE RUNS MULTIPLES
# ==============================
