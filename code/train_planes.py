...
# Le YAML pointe desormais vers les donnees filtrees (Avions uniquement)
DATA_INIT_YAML = Path("/linux/antoimartin/v2/data_airplanes.yaml").resolve()
...
def train_golden_model(model_type, imgsz, batch, device):
    ...
    # Nom specifique pour les avions
    run_name = f"baseline_airplanes_20pct_{Path(model_type).stem}_{imgsz}"

    try:
        results = model.train(
            data=str(DATA_INIT_YAML),
            epochs=200,            # Laisse de la marge pour bien converger
            imgsz=imgsz,           # Garde la resolution native demandee
            batch=batch,           # Securite OOM
        ...
        print(f"\n[SUCCES] Modele de depart (AL Baseline Airplanes) sauvegarde ici : {best_pt}")
...
def main():
    parser = argparse.ArgumentParser(description="Script pour entrainer le modele initial AVIONS (20%) avant Active Learning.")
    ...
    if args.mode == "hr":
        print("\n=== Entrainement Modele INITIAL HR (YOLOv8-Large sur 20% - AVIONS) ===")
        # Batch=4 est plus safe pour 1024px/Large sur 11GB VRAM. 
        train_golden_model(model_type="yolov8l.pt", imgsz=1024, batch=4, device=device_arg)

    elif args.mode == "lr":
        print("\n=== Entrainement Modele INITIAL LR (YOLOv8-Small sur 20% - AVIONS) ===")
        # Modele petit (Small) + Image plus petite (640px) = on peut monter le batch size
        train_golden_model(model_type="yolov8s.pt", imgsz=640, batch=32, device=device_arg)
