...
# Modele initial (Baseline 20%, 37 classes)
...
RESULTS_DIR = BASE_DIR / "code/AL_Final_Showdown"
...
# --- PARAMETRES DE COUT (TEMPS HUMAIN) ---
...
# ==============================
def execute_run(strategy, budget_minutes, missions, imgsz, device, run_id, all_images):
    print(f"\n--- SHOWDOWN RUN {run_id} ({strategy.upper()} - {budget_minutes}min) ---")
    exp_dir = RESULTS_DIR / f"{strategy}_{budget_minutes}min" / f"run_{run_id}"
    ...
    print(f"[INIT] Evaluation de la Baseline...")
    ...
    for m in range(missions):
        ...
        print(f"[SELECTION] {len(selected)} tuiles (Cout estime: {time_spent/60:.1f} min)")
...
def main():
    ...
    print("="*60)
    print(f"LE GRAND FINAL : {args.strategy.upper()} - {budget} min")
    print("="*60)
    ...
    final_dir = RESULTS_DIR / f"{args.strategy}_{budget}min"
